...
- 移除聊天記錄外框左下、右下兩個花卉裝飾，只保留金色雙線與上方淡淡的酒紅光暈。
- 歡迎頁不再是灰色整塊面板：版本獨立固定在左上，最近聊天以更寬鬆的無底色分列呈現；放大角色頭像與角色卡標題，只顯示角色卡標題，原有日期與操作按鈕仍可使用。
- 輸入列往下調整，並在外框內預留底部空間，避免金色輸入框邊界被外殼吃掉。

## v1.12：重新收斂歡迎頁字級與設定頁金屬光

- 歡迎／最近聊天總覽降回介面尺度：角色頭像與角色卡標題仍被凸顯，但不再大到像告示牌；標題與角色名字使用中文襯線、英文花體的字體堆疊。
- 「聊天補全預設設定檔」標題改成與已美化設定頁相同的橫掃金屬光樣式。
- 世界／知識書的「點選展開」、擴充功能內容，以及使用者設定裡指定的段落標題，統一套用更精緻的金屬掃光與摺疊列樣式。
